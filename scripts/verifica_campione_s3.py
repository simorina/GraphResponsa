...
CHIAVI = ["id", "preambolo", "citazioniPreambolo", "partizioni", "articoli",
          "allegatoPostPromulgazione", "convenzioneNelDispositivo",
          "allegatoHaStrutturaPropria"]
...
for nid, nota in CAMPIONE.items():
    ...
    att = ATTESI.get(nid)
    if att:
        if len(art) != att["articoli"]:
            problemi.append(f"articoli {len(art)} invece di {att['articoli']}")
...
parti = sorted({a.get("parte") for a in d["articoli"]} - {None})
print(f"L-0-1910 parti riconosciute: {parti}, articoli {len(d['articoli'])}")
d = json.loads((DEST / "X-3-1942.json").read_text(encoding="utf-8"))
print(f"X-3-1942 articoli {len(d['articoli'])}, convenzione "
      f"{len(d.get('convenzioneNelDispositivo') or '')} caratteri")
