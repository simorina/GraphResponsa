...
def parsa_e_scrivi(nid, prefisso):
    def lavoro():
        ...
        return {
            ...
            "ripetuti": sum(1 for a in dati["articoli"] if "-rip" in str(a["numero"])),
            "articoliAllegato": sum(1 for a in dati["articoli"] if a.get("allegato")),
            "commiAllegato": sum(1 for a in dati["articoli"] for c in a["commi"]
                                 if c.get("parte") == "allegato"),
        }
...
def main():
    ...
    print(f"documenti a struttura dedotta {sum(1 for r in ok if r['dedotta'])}, "
          f"con articoli -ripN {sum(1 for r in ok if r['ripetuti'])}, "
          f"con articoli d'allegato {sum(1 for r in ok if r['articoliAllegato'])}, "
          f"con parti d'allegato nei commi {sum(1 for r in ok if r['commiAllegato'])}")
