...
> **Stato attuale: i fix sono su main (branch `parser-unificato`, 24/09).**
> Le sezioni da 1 a 11 descrivono i fix come erano sul branch `fix-parser`.
> Portandoli su main alcuni sono cambiati: leggere prima la sezione
> [Integrazione su main](#integrazione-su-main-2409).

## Integrazione su main (24/09)

Fra il 16 e il 18/09 main aveva lavorato sullo stesso parser da un'altra
parte: `RE_ARTICOLO_ESTESO` (rubrica sulla riga), `_indice_iniziale()`,
`recupera_orfane()`, `RE_CITAZIONE` riscritta, e soprattutto tre moduli che
riordinano la lettura dopo il parser: `src/articoli.py`, `src/commi.py`,
`src/allegati.py`. La differenza di fondo: **main tiene gli allegati nel
grafo** (articoli `all-N`, parti d'allegato `all1` nei commi, tabelle `all-A`),
il branch li toglieva. I fix sono stati quindi riscritti su main uno per uno,
non fusi, e dove toccavano gli allegati hanno preso la forma di main.

| § | Fix | Su main |
|---|---|---|
| 1 | CAUSA-A | Non si taglia piu'. `_cerca_promulgazione()` (stessa regex, stesso `match()`) segna la fine del dispositivo. Il testo dopo le firme apre un comma nuovo, che `commi.py` marca parte d'allegato `all1`, ma solo oltre `MIN_ALLEGATO` = 200 caratteri: sotto ci sono residui delle firme (1.527 documenti), sopra tabelle, accordi, regolamenti (1.375). Gli articoli dopo la formula intera diventano `all-N` in `articoli.py` anche quando i numeri non si ripetono: DC-52-2016 e' `Unico` piu' `all-1`...`all-20`. `RE_PROMULGAZIONE` sta in `commi.py`. |
| 2 | 1.1 rubrica presunta | Invariato. |
| 3 | Varianti d'intestazione | `RE_ARTICOLO` come sul branch, ma solo prima della firma: dopo resta quella di main (`RE_ARTICOLO_DOPO_FIRMA`). Il branch non leggeva mai oltre la firma; su main, negli allegati di DC-36-2020 e L-108-2011, "Art 13" senza punto creava rinvii-articolo fino allo scarto di `03_load.py`. |
| 4 | Id duplicati, secondo atto | Gia' risolto da main (`-ripN`, `all-N`), non portato: niente `id_articolo()`, `_marcatore_atto()`, campi `parte` e `idResoUnivoco`. L-0-1910 ha `1-rip2` invece di `reg/art-1`; 0 id duplicati come sul branch. |
| 5 | A, tipi di citazione | Aggiunti all'alternanza della regex riscritta da main. Prove in `_PROVE_TIPI_CITAZIONE` (`_PROVE_CITAZIONE` e' quella di main). |
| 6 | B, intestazione minuscola | Solo dentro un articolo aperto e prima della firma. Dopo la firma "art. 166" e' la voce di una tabella che `allegati_tabellari()` rilegge; prima del primo articolo il testo finirebbe nel preambolo, fuori dalla ricerca (DD-12-2017, che comincia con la tabella). |
| 7 | C, convenzione | Non si taglia: `articoli._convenzione()` la riconosce sugli articoli, con le stesse tre condizioni, e i suoi articoli diventano `all-N`. X-3-1942: `Unico` piu' `all-1`...`all-23`. |
| 8 | D, errata corrige | Invariato. |
| 9 | P1, articoli citati | Invariato, con tre adattamenti. Le intestazioni estese in sequenza contano nella numerazione. Il blocco senza rientro si ferma alla formula (dopo c'e' l'allegato). `articoli._dividi()` separa un'intestazione rimasta in coda a un comma solo se il numero e' quello in corso o il successivo: senza, rifaceva articoli di L-24-2022 i 58-bis...58-nonies citati. |
| 10 | 1.3 commi con suffisso | Invariato. Un comma con suffisso fra virgolette di una novella diventa capoverso (`commi.py`): 129 dei 526 del branch. |
| 11 | 1.2 `Art. 5-bis` | Invariato. |

Un aggiustamento in `allegati.py`: il comma della formula riunisce i
firmatari scritti prima della tabella, altrimenti D-192-2005 e D-37-2004
perdevano i nomi dei Segretari quando il parser apre il comma dell'allegato.

### Verifica sul corpus

Parser di main e parser integrato sugli stessi 11.122 documenti (gli
Statuti sono esclusi come in `02_parse.main()`), righe estratte una volta da
S3:

| Metrica | main | integrato |
|---|---|---|
| Errori di parsing | 0 | 0 |
| Documenti con id duplicati (articoli o commi) | 0 | 0 |
| Parole presenti su main e assenti nell'integrato | — | **0** |
| Scartati da `03_load.py` (numerazione patologica) | 2 | 3 (+ L-108-2011, LIMITI_NOTI 12) |
| Documenti instabili se si riapplica il riordino (14, 15) | 0 | 0 |
| Articoli | 77.739 | 77.508 |
| Articoli d'allegato | 5.472 | 10.281 |
| Commi | 169.907 | 171.871 |
| Parti d'allegato nei commi | 15.168 | 16.516 |
| Commi con suffisso | 0 | 397 (piu' 129 capoversi) |
| Articoli -ripN | 508 | 403 |
| Articoli con rubrica | 38.847 | 34.955 |
| Citazioni | 78.319 | 79.463 (94 dei tipi nuovi) |
| Errata corrige con articoli propri | 31 | 0 |
| Documenti a struttura dedotta | 671 | 700 |

Le rubriche in meno sono 3.640 frasi che main prendeva per rubrica
("Il presente Decreto entra immediatamente in vigore") e che tornano testo
(fix 1.1); le altre sono di articoli rinominati `all-N` o spariti perche'
citati. Le citazioni perse sono 7 in 6 documenti, tutte lette da un'intestazione
che main lasciava nel testo ("Art. 16-bis (Modifiche dell'articolo 19 del
Decreto-Legge...)") e che ora e' rubrica.

Documenti di riferimento, main → integrato: L-162-2004 38 → 9 articoli,
L-168-2005 56 → 17 (piu' 7 d'allegato), L-24-2022 53 → 40, L-33-1988 34 → 28,
DD-204-2020 16 → 4, DD-19-2016 20 → 27, DD-12-2017 30 → 4, L-87-2026 240 → 245
commi. Restano come su main D-100-1995 (lo statuto, ora `all-N`), L-21-1981
(21), R-1-2004 (56), le tabelle di DD-149-2016 e D-11-2000.

**Il grafo non e' stato toccato.** Rispetto a main cambia la lettura di 3.598
documenti: 2.045 solo nelle rubriche, 1.862 nel testo dei commi, 476 negli id
degli articoli. Per portarli nel grafo vale la procedura di main (`02_parse.py
--atti`, `15_ricostruisci_articoli.py --atti` e i passi che il suo docstring
elenca). `04_verify.py` si aspetta 245 commi in L-87-2026: fallisce finche'
quell'atto non e' ricaricato.
...
scritte. Su main: `RE_ARTICOLO` → `RE_ARTICOLO_DOPO_FIRMA` →
`RE_ARTICOLO_ESTESO` → `RE_COMMA` → `RE_CITAZIONE` → `RE_BERSAGLIO` →
`estrai_citazioni()` → `_indice_iniziale()` → promulgazione → rubrica presunta
→ intestazione minuscola → testo citato → errata corrige → `parse()` → prove a
livello di `parse()`. Convenzione e allegato dopo la firma stanno in
`src/articoli.py` (`_convenzione`, `_allegato_certo`), con le prove in
`_PROVE_PARSE`.
...
> Superata dall'integrazione su main: quei campi non esistono piu', gli
> allegati sono articoli `all-N` e parti d'allegato che `03_load.py` carica
> gia'. La nota resta per chi legge la storia del branch.
